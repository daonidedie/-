...
public partial class file_SuSuCongBookList : System.Web.UI.Page
{
    ...
    void BindData(string key = null)
    {
        ...
        if (key == null)
        {
            if (GetBooksType() != "-3")
            {
                ...
                if (decimal.TryParse(GetBooksType(), out lx))
                {
                    ...
                    if (lx > 0)
                    {
                        if (QliliHelper.Books.Count > 0)
                        {
                            ListSource = QliliHelper.Books.Where(p => p.分类表ID == lx && p.最新章节 != null && p.包含有效章节 != null && p.包含有效章节 > 0).OrderByDescending(p => p.最后更新时间).AsQueryable();
                        }
                        else
                        {
                    ...
                    if (lx == -1)
                    {
                        ...
                        if (QliliHelper.Books.Count > 0)
                        {
                            ListSource = QliliHelper.Books.Where(p => p.完本 && p.包含有效章节 > 0).OrderByDescending(p => p.最后更新时间).AsQueryable();
                        }
                        else
                        {
            ...
            else
            {
                if (QliliHelper.Books.Count > 0)
                {
                    ListSource = QliliHelper.Books.Where(p => p.最新章节 != null && p.包含有效章节 != null && p.包含有效章节 > 0).OrderByDescending(p => p.最后更新时间).AsQueryable();
                }
                else
                {
        ...
        else
        {
            if (QliliHelper.Books.Count > 0)
            {
                ListSource = QliliHelper.Books.Where(p => p.书名.Contains(key) || p.作者名称.Contains(key)).OrderByDescending(p => p.最后更新时间).AsQueryable();
            }
            else
            {
