/******************************************************
* 文件名：CodeCharSet.cs
* 文件功能描述：web数据操作 通用 类型 命名空间 CodeCharSet
* 
* 创建标识：周渊 2007-8-18
* 
* 修改标识：周渊 2008-4-26
* 修改描述：按代码编写规范改写部分代码
* 修改标识：周渊 2011-10-26
* 修改描述：增加了 ToPingYing 方法 并将些类改成String的扩展方法
******************************************************/
...
namespace Skybot.Tong.CodeCharSet
{
    #region 编码转换,GB2312转拼音

    /// <summary>
    /// 中文变拼音
    /// </summary>
    public static class ConvertHzToPz_Gb2312
    {
...
new string[]{"Zuo","做作坐左座昨凿琢撮佐笮酢唑祚胙怍阼柞乍侳咗岝岞挫捽柮椊砟秨稓筰糳繓苲莋葃葄蓙袏諎醋鈼鑿飵嘬阝"}
        ...
        /// </summary>
        /// <param name="str"> 传入字符串</param>
        /// <returns>拼音字母</returns>
        public static string ToPingYing(this string str)
        {
            str = new string(str.Distinct().ToArray());
            //这里有一个已经添加了相同项的异常所以需要过滤
            //结果
            Dictionary<char, string> result = str.ToDictionary(p => p, (c) => "");
            //循环得到输出的结果
            str.ToList().ForEach((c) =>
            {
                foreach (var val in _Allhz)
                {
                    if (val[1].Contains(c))
                    {
                        result[c] = val[0];
