                ...
                switch (result[0])
                {
                    case RecordType.Page:
                        ...
                        if (records.Count() > 0)
                        {
                            ...
                            #region 初始化页面对象数据
                            //如果当前缓存记录中存在这条记录
                            if (!BooksCache.ContainsKey(record.书名表.GUID))
                            {
                                //页面所在的分类
                                ...
                                //尝试添加到数据中
                                ClassesCache[record.书名表.分类表.ID].Books.TryAdd(record.书名表.GUID, book);
                                ...
                                if (!book.Pages.ContainsKey(sign))
                                {
                                    ...
                                    //将页面添加到缓存集合
                                    book.Pages.TryAdd(record.本记录GUID, page);
                                }
                                //添加到书的缓存集合
                                BooksCache.TryAdd(record.书名表.GUID, book);
                            }
                            #endregion

                            //得到书本对象的数据
                            page = BooksCache[record.书名表.GUID].Pages[record.本记录GUID];
                            page.Record.总访问次数++;
                    ...
                    case RecordType.Book:
                        ...
                        if (bookrecords.Count() > 0)
                        {
                            ...
                            #region 初始化页面对象数据
                            //如果当前缓存记录中存在这条记录
                            if (!BooksCache.ContainsKey(record.GUID))
                            {
                                //页面所在的分类
                                TabClassEntity Typeclass = null;
                                //如果包括分类
                                ...
                                BooksCache.TryAdd(record.GUID, book);
                            }
                            //更新状态
                            BooksCache[record.GUID].Modifyd = true;
