...
namespace Skybot.Collections.Analyse
{
    ...
    public class SingleListPageAnalyse : IAnalyse
    {
        ...
        void Analyse()
        {
            ...
            //开始循环子原素
            AnalyseMaxATagNearest(xdom.Root, possiblyResultElements, 0, new PossiblyResultElement()
            {
            ...
            var allAElements = from a in possiblyResultElements
                               ...
                               {
                                   Url = new Uri(baseUri, a.CurrentElement.Attribute("href").Value),
                                   Title = a.CurrentElement.Value.WordTraditionalToSimple(),
                                   index = System.Text.RegularExpressions.Regex.Match(a.CurrentElement.Value.WordTraditionalToSimple(), @"第[壹一两贰二叁三肆四伍五陆六柒七捌八玖九拾十佰百仟千萬万零|(0-9)]{1,}章").Value.Replace("第", "").Replace("章", "").ConverToDigit(),
                                   Indexs = GetTitleIndexs(System.Text.RegularExpressions.Regex.Match(a.CurrentElement.Value.WordTraditionalToSimple(), @"第[壹一两贰二叁三肆四伍五陆六柒七捌八玖九拾十佰百仟千萬万零|(0-9)]{1,}章").Value.Replace("第", "").Replace("章", "").ConverToDigit())
                               };
