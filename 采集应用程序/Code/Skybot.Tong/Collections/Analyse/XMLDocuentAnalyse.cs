...
namespace Skybot.Collections.Analyse
{
    ...
    public class XMLDocuentAnalyse : IAnalyse
    {
        ...
        public string GetContent(string Content)
        {
            ...
            if (PathExpression != "" && PathExpression != null)
            {
                ...
                HtmlNode node = dom.DocumentNode.SelectSingleNode(PathExpression);
                if (node != null)
                {
                    //得到当前数据的结果
                    string str = node.InnerHtml;
                    htmlDom = null;
                    node.InnerHtml = string.Empty; 
                    node = null;
                    dom.DocumentNode.InnerHtml = string.Empty;
                    return str;
                }
            ...
            htmlDom.DocumentNode.InnerHtml = string.Empty;
            htmlDom = null;
        ...
        {            //如果为 null 直接返回
            ...
            entity.BaseUrlObject = null;
            entity.HtmlDocument = null;
            entity.OriginContent = null;
            entity.PageTitle = null;
            entity.IsGetContentSuccess = false;
            entity.IndexPageUrl = null;
            entity.XmlParseConten = null;
        ...
        string GetMainContentExpression(XMLDocuentAnalyseEntity domEntity)
        {
            ...
            if (MC.Count > 0)
            {
                ...
                try
                {
                    ...
                    var list = matchResluts.Where(p => p.StrinLength >= TextValidLength);
                    if (list.Count() > 0)
                    {
                        list.Aggregate((curr, next) =>
                       {
            ...
            #region 正则表达式线程CPU 100% 优化方法
            //放在一个线程里,如果超过3秒线程还没有执行完成则直接关闭线程
            //这样可以保证CPU100%的情况得到改善
            System.Threading.Thread thread = new System.Threading.Thread(new System.Threading.ThreadStart(() =>
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine(DateTime.Now + "提取数据" + ex.Message + "|||||" + ex.StackTrace);
                }
            }));
            thread.Start();
            //task只是做一个等待器
            ...
            //如果3秒还没有分析出来正则表达式结果
            if (!task.Wait(1000 * 3) && string.IsNullOrEmpty(mainContont))
            {
                thread.Abort();
                return mainContont;
            }
    ...
    public class XMLDocuentAnalyseEntity
    {
        ...
        internal HtmlDocument HtmlDocument { get; set; }
