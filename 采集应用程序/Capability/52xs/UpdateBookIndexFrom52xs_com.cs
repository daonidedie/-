...
namespace 采集应用程序.Capability
{
    ...
    /// 从86中文网更新书的索引
    /// </summary>
    public class UpdateBookIndexFromxs52_com : System.Windows.DependencyObject, System.ComponentModel.INotifyPropertyChanged
    {
            ...
            System.Threading.Tasks.Task task = System.Threading.Tasks.Task.Factory.StartNew(new Action(delegate
               {
                   using (TygModel.Entities tygdb = new TygModel.Entities())
                   {
                       ...
                       string[] arr =new BookListxs52_com().arrTypes;
                       foreach (string  classid in arr)
                       {

                           string currentSorturl = classid.Contains("fullflag=1") ? classid.Split(',')[1] + "{0}" : classid.Split(',')[1] + "{0}.html";


                           System.Diagnostics.UDPGroup.SendStrGB2312("开始采集图书列表" +currentSorturl);
                           ...
                           Task<IEnumerable < Skybot.Collections.Sites.AbstractBookInfo >> taskClass = Task.Factory.StartNew<IEnumerable<Skybot.Collections.Sites.AbstractBookInfo>>(
                               () =>
                               {
                                   ...
                                   var al = new Skybot.Collections.Sites.BookListxs52_com() { BaseUrl = currentSorturl }.DoWork();
                                   return al;
                               });
                           try
                           {
                           ...
                           for (int k = 0; k < AllBooks.Count(); k++)
                           {
                               try
                               {
                                       ...
                                       ))
                                   {
                                       ...
                                       using (TygModel.Entities db = new Entities())
                                       {
                                           //得到当前书更新记录
                                           TygModel.书名表 currentBook = db.书名表.Where(p => p.书名.Replace("》", "").Replace("《", "").Trim() == bookName.Replace("》", "").Replace("《", "").Trim() && p.作者名称.Trim() == creter.Trim()).FirstOrDefault();
                                           ...
                                           currentBook.最后更新时间 = DateTime.Now;
